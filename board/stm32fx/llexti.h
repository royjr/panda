...
void exti_irq_init(void) {
  ...
  // CAN1_RX IRQ on falling edge (EXTI5)
  current_board->enable_can_transceiver(2U, false);
  SYSCFG->EXTICR[1] &= ~(SYSCFG_EXTICR2_EXTI5_Msk);
  SYSCFG->EXTICR[1] |=  (SYSCFG_EXTICR2_EXTI5_PB);
  EXTI->IMR  |=  EXTI_IMR_MR5;
  EXTI->RTSR &=  ~EXTI_RTSR_TR5; // rising edge
  EXTI->FTSR |=  EXTI_FTSR_TR5; // falling edge
...
bool check_exti_irq(void) {
  return ((EXTI->PR & EXTI_PR_PR8) || (EXTI->PR & EXTI_PR_PR5) || (EXTI->PR & EXTI_PR_PR3) || (EXTI->PR & EXTI_PR_PR0));
}
